...
export class BlogService {
  ...
  async createBlog(
    ...
    try {
      ...
      if (
        args.status === BlogStatus.PUBLISHED &&
        (!args.categoryIds || args.categoryIds.length === 0)
      ) {
        if (!categories?.length)
          this.exceptionService.sendNotFoundException(
            `Invalid category ids: ${uniqueCategoryIds}`,
          );
      ...
      const blog = new Blog({
        ...
        seoDescription:
          args.seoDescription ||
          args.excerpt ||
          this.extractExcerpt(args.content),
        tags: args.tags || [],
      ...
      if (Object.entries(files)?.length) {
        blog.media = {};
        const keys = await this.sharedService.uploadMultipleFileToS3Bucket(
          files,
          args.dims,
        );
        blog.media = {
          images: await this.sharedService.getMultipleFilesFromS3Bucket(keys),
        };
  ...
  async updateBlog(
    ...
    try {
      ...
      if (blog.adminId?.id !== adminId) {
        this.exceptionService.sendForbiddenException(
      ...
      if (newStatus === BlogStatus.PUBLISHED) {
        ...
        if (categoryIds.length === 0) {
          this.exceptionService.sendNotAcceptableException(
            RESPONSE_MESSAGES.CATEGORY_REQUIRED_FOR_BLOG,
          );
        }
      }
      ...
      if (args.slug) {
        ...
        if (existingBlog && existingBlog.id !== blog.id) {
          this.exceptionService.sendConflictException(
            RESPONSE_MESSAGES.BLOG_SLUG_ALREADY_EXIST,
          );
        }
        blog.slug = args.slug;
      }
      ...
      if (args.mediaToBeRemoved) {
        ...
        if (
          args.mediaToBeRemoved.imageKeys?.length &&
          Object.entries(blog.media?.images || {}).length
        ) {
          ...
          args.mediaToBeRemoved.imageKeys.forEach((key) => {
            if (contentImagesKeys?.[key]) {
              ...
              this.logger.log(
                `Removed image key "${key}" from blog ${args.id}`,
                this.updateBlog.name,
              );
            }
        ...
        if (bucketKeysToDelete.length) {
          ...
          this.logger.log(
            `Deleted ${bucketKeysToDelete.length} files from S3 for blog ${args.id}`,
            this.updateBlog.name,
          );
        } else {
          this.exceptionService.sendBadRequestException(
            RESPONSE_MESSAGES.MEDIA_NOT_FOUND,
          );
        }
      ...
      if (Object.entries(files)?.length) {
        ...
        if (Object.entries(files)?.length) {
          ...
          if (
            ...
            this.logger.error(
              `Image limit exceeded (current: ${Object.entries(blog.media?.images || {})?.length}, new: ${Object.entries(files)?.length})`,
              this.updateBlog.name,
            );
            this.exceptionService.sendNotAcceptableException(
              RESPONSE_MESSAGES.IMAGE_LIMIT_EXCEEDED,
            );
          }
          const fileKeys = Object.keys(files);
          const mediaImagesKeys = Object.keys(blog.media?.images || {})?.map(
            (key) => key.split('_')[0],
          );
          fileKeys.map((key) => {
            if (mediaImagesKeys.includes(key)) {
              this.exceptionService.sendConflictException(
                `Image with key: ${key} already exists in blog media`,
              );
            }
          });
          // Upload new images
          ...
          const keyValues =
            await this.sharedService.getMultipleFilesFromS3Bucket(keys);
          blog.media = {
            ...blog.media,
            images: { ...blog.media?.images, ...keyValues },
          };
          this.logger.log(
            `Uploaded ${Object.entries(files)?.length} new images for task post ${args.id}`,
            this.updateBlog.name,
          );
        }
        if (!blog.mediaExpiredAt)
          blog.mediaExpiredAt = new Date().setDate(new Date().getDate() + 6);
      }
  ...
  async deleteBlog(id: number, adminId: number) {
    try {
      ...
      if (Object.entries(blog?.media?.images || {}).length) {
        const bucketKeys: string[] = Object.keys(blog.media?.images || {})?.map(
          (key) => key,
        );
        if (bucketKeys.length)
          await this.sharedService.deleteFilesFromS3Bucket(bucketKeys);
      }
  ...
  async listBlogs(args: BlogListingDTO) {
    try {
      ...
      if (tags?.length) {
        queryBuilder.andWhere(
          tags.map((_, index) => `blog.tags ILIKE :tag${index}`).join(' OR '),
      ...
      queryBuilder.orderBy('blog.createdAt', 'DESC').skip(skip).take(pageSize);

      const [blogs, total] = await queryBuilder.getManyAndCount();

      if (args.id && blogs[0]?.categories?.length)
        blogs[0].categories = (await this.categoryRepo.findBy({
  ...
  async publishBlog(id: number, adminId: number) {
    try {
      ...
      return this.sharedService.sendResponse(RESPONSE_MESSAGES.BLOG_PUBLISHED, {
        blog,
      });
    } catch (error) {
  ...
  async unpublishBlog(id: number, adminId: number) {
    try {
      ...
      return this.sharedService.sendResponse(
        RESPONSE_MESSAGES.BLOG_UNPUBLISHED,
        { blog },
      );
    } catch (error) {
  ...
  async selectCategories(args: SelectCategoriesDTO, userId: number) {
    try {
      ...
      const userCategoriesToUpsert = categories.map(
        (category) =>
          new UserCategory({
            userId,
            categoryId: category,
      ...
      await this.userCategoryRepo.upsert(userCategoriesToUpsert, [
        'userId',
        'categoryId',
      ]);

      return this.sharedService.sendResponse(
        RESPONSE_MESSAGES.CATEGORIES_SELECTED,
        { userCategories: userCategoriesToUpsert },
      );
    } catch (error) {
  ...
  async getUserSelectedCategories(userId: number) {
    try {
      ...
      return this.sharedService.sendResponse(
        RESPONSE_MESSAGES.USER_CATEGORIES_LISTING,
        { userCategories },
      );
    } catch (error) {
    ...
    try {
      ...
      if (args.sortBy) {
        queryBuilder.orderBy(
          'blog.publishedAt',
          args.sortBy === BlogSortBy.NEW_TO_OLD ? 'DESC' : 'ASC',
        );
      }

      queryBuilder.skip(args.pageNumber).take(args.pageSize);

      const [blogs, total] = await queryBuilder.getManyAndCount();

      // Enrich blogs with categories if IDs are provided
      if (
        (args.ids?.length === 1 || args.id) &&
        blogs?.[0]?.categories?.length
      ) {
        blogs[0].categories = (await this.categoryRepo.findBy({
          id: In(blogs[0].categories),
        })) as any;
  ...
  async getUserBlogCategories(userId: number) {
    try {
      ...
      const userCategories =
        userCategoriesWishlist?.map((uc) => uc.categoryId) || [];
  ...
  async userWishlistToggle(args: IdDTO, user: User) {
    try {
      ...
      const userWishlist = await this.userWishlistService.userWishlistToggle(
        args.id,
        user,
        'blog',
      );
      return this.sharedService.sendResponse(
        RESPONSE_MESSAGES.WISHLIST_UPDATED_SUCCESSFULLY,
        { userWishlist },
      );
    } catch (error) {
